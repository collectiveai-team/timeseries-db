...
from tsdb.decorators.pydantic_decorator import timescale_crud, create_crud_session
...
@timescale_crud(
    ...
    enable_audit=True,
    create_hypertable=False,  # Regular table for this example
)
...
def demonstrate_sqlmodel_syntax():
    ...
    with create_crud_session(DATABASE_URL) as session:
        ...
        user = User(
            username="alice_sqlmodel",
            email="alice@sqlmodel.com"
        )
        ...
        user2 = User(
            username="bob_sqlmodel", 
            email="bob@sqlmodel.com"
        )
